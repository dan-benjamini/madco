function [ImageRef,Image, bvalOreintAvg,TI ]=read_2dseq_3D_DiffIsoAcqIRpath(im_ind,pathname)
% 
% close all
% clear all
% clc
...
[TI, ~]=find_multi_method_mod(pathname,im_ind(1),'PVM_SelIrInvTime');
...
if A0
    for ii=1:num_dir
        bvalOreintAvgMat(:,ii)=bval((bvalPerNum*(ii-1)+2):(ii*bvalPerNum)+1);
        Image(:,:,:,:,ii)=ImagePre(:,:,:,(bvalPerNum*(ii-1)+1):ii*bvalPerNum);
        
    end
else
    for ii=1:num_dir
        bvalOreintAvgMat(:,ii)=bval((bvalPerNum*(ii-1)+1):(ii*bvalPerNum));
        Image(:,:,:,:,ii)=ImagePre(:,:,:,(bvalPerNum*(ii-1)+1):ii*bvalPerNum);
